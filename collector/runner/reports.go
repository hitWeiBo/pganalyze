...
func getRequestedReports(server *state.Server, globalCollectionOpts state.CollectionOpts, logger *util.Logger) (requestedReports []reports.Report, grant state.Grant, err error) {
	...
	requestedReport, err := reports.InitializeReport("bloat", "1")
	requestedReport1, err := reports.InitializeReport("buffercache", "2")
	requestedReport2, err := reports.InitializeReport("vacuum", "3")
	requestedReport3, err := reports.InitializeReport("sequence", "4")
	requestedReports = append(requestedReports, requestedReport)
	requestedReports = append(requestedReports, requestedReport1)
	requestedReports = append(requestedReports, requestedReport2)
	requestedReports = append(requestedReports, requestedReport3)
