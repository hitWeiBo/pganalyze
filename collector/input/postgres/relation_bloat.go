		...
		SELECT table_schema,
		  ...
		  FROM columns
	 	  JOIN pg_catalog.pg_stat_user_tables psut ON (table_schema = psut.schemaname AND table_name = psut.relname)
			LEFT OUTER JOIN (SELECT * FROM pganalyze.get_column_stats()) pg_stats ON (table_schema = pg_stats.schemaname AND table_name = pg_stats.tablename AND column_name = attname)
...
null_headers AS (
		...
		SELECT
				...
				tablename,
				hdr, ma, bs
		FROM (SELECT * FROM pganalyze.get_column_stats()) pg_stats CROSS JOIN constants
...
index_item_sizes AS (
	SELECT ia.nspname,
		...
		JOIN (SELECT * FROM pganalyze.get_column_stats()) s ON (s.schemaname = ia.nspname
...
func GetRelationBloat(logger *util.Logger, db *sql.DB, columnStatsSourceTable string, ignoreRegexp string) (relBloat []state.PostgresRelationBloat, err error) {
	...
	rows, err := db.Query(QueryMarkerSQL+tableBloatSQL, ignoreRegexp)
	if err != nil {
...
func GetIndexBloat(logger *util.Logger, db *sql.DB, columnStatsSourceTable, ignoreRegexp string) (indexBloat []state.PostgresIndexBloat, err error) {
	rows, err := db.Query(QueryMarkerSQL+indexBloatSQL, ignoreRegexp)
...
func GetBloatStats(logger *util.Logger, db *sql.DB, systemType, ignoreRegexp string) (report state.PostgresBloatStats, err error) {
	...
	if StatsHelperExists(db, "get_column_stats") {
		...
		columnStatsSourceTable = "(SELECT * FROM pganalyze.get_column_stats()) s"
	} else {
