...
const activitySQL string = `SELECT (extract(epoch from COALESCE(backend_start, pg_catalog.pg_postmaster_start_time()))::int::text || right(pid, 6))::bigint, 
				datid, datname, usesysid, usename, right(pid, 6), application_name, client_addr::text, client_port,
